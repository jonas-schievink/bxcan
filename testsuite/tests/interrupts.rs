...
mod tests {
    ...
    fn rx_interrupt_message_pending(state: &mut State) {
        ...
        irq::scope(|scope| {
            ...
            defmt::unwrap!(nb::block!(m.lock(|state| state.can1.transmit(&frame))));
    ...
    fn sce_interrupt_sleep(state: &mut State) {
        state
            .can1
            .enable_interrupts(Interrupts::SLEEP | Interrupts::WAKEUP);
        ...
        irq::scope(|scope| {
            ...
            m.lock(|state| state.can1.wakeup());
            //defmt::assert!(wakeup_interrupt_fired.load(Ordering::Relaxed));
        });

        state
            .can1
            .disable_interrupts(Interrupts::SLEEP | Interrupts::WAKEUP);
    }
    ...
    fn sce_interrupt_wakeup(state: &mut State) {
        ...
        irq::scope(|scope| {
            ...
            let frame = Frame::new_data(StandardId::new(0).unwrap(), []);
            defmt::unwrap!(nb::block!(m.lock(|state| state.can2.transmit(&frame))));
            m.lock(|state|
                // Wait until all transmissions have completed.
